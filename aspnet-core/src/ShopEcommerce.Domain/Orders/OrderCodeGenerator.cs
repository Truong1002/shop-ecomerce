...
namespace ShopEcommerce.Orders
{
    public class OrderCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(ShopEcommerceConsts.OrderIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(ShopEcommerceConsts.OrderIdentitySettingId, "Sản phẩm", ShopEcommerceConsts.ProductIdentitySettingPrefix, 1, 1));
